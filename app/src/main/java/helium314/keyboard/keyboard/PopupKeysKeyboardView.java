...
public class PopupKeysKeyboardView extends KeyboardView implements PopupKeysPanel {
    ...
    private void showPopupKeysPanelInternal(final View parentView, final Controller controller,
        ...
        // The panel's own drawable is a white shape that is only ever the right colour because a
        // colour filter is put on it, so re-apply that from the colours in force right now. Doing
        // it here rather than trusting what setKeyboard() left behind means the panel cannot come
        // up white, whatever else has touched the drawable in between.
